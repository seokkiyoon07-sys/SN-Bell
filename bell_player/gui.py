...
class BellGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("수능_ bell_player_ver.1 by SN독학기숙학원")
        self.sched = None
        ...
        self.manual_check_vars = [tk.BooleanVar(value=True) for _ in range(31)]
        self.manual_thread = None
        self.manual_stop_event = threading.Event()

        # Labels for each index (01~31)
        self.index_labels = [
            "01-0805_1 인수",
            "02-0810_1 입실",
            "03-0825_1 예비",
            ...
            "27-1535_4 탐구1본령",
            "28-1600_4 탐구1종료5분전",
            "29-1605_1607_4 탐구1종료-4탐구2본령",
            "30-1632_4 탐구2종료5분전",
            "31-1637_4 탐구2종료_4교시시험장",
        ]
        ...
        # Scrollable container for the 31 checkboxes
        ...
        # create 31 checkboxes vertically (single column)
        ...
        tk.Label(frm, text="bell-player developed by SN독학기숙학원", fg="#666666").grid(row=9, column=0, columnspan=3, sticky="w", pady=(4,0))
        ...
        help_content = (
            ...
            "- 사운드 폴더: 벨 음원 위치를 지정합니다. (예: C\\code\\SN-Bell\\bell_sound)\n"
            "- 볼륨: 0~1 사이 값. 일부 포맷은 OS 볼륨에 따릅니다.\n"
            "- 테스트 모드: 1~31을 3초 간격으로 빠르게 재생 스케줄링합니다.\n"
            "- (옵션) MCI 우선 사용: 메인 탭의 체크박스로 전환할 수 있습니다(기본: 켜짐).\n\n"
            "2) 스케줄러\n"
            "- 스케줄러 시작: schedule.csv(또는 테스트 모드) 기반으로 오늘 남은 시간만 예약합니다.\n"
            ...
            "4) 파일명/폴더\n"
            "- 권장 파일명: 01.mp3 ~ 31.mp3 (다른 확장자도 자동 탐색).\n"
    ...
    def select_defaults(self):
        defaults = self.config.get("default_manual_indices") or [5, 7, 11, 13, 16, 18, 22, 24, 27, 29, 31]
        defaults = [int(x) for x in defaults]
        for i, var in enumerate(self.manual_check_vars):
            var.set((i + 1) in defaults)
    ...
    def open_clock_window(self):
        ...
        try:
            ...
            pass
        self.clock_win.geometry("360x140")
        ...
        def tick():
            try:
                ...
                now = datetime.now()
                self.clock_label.configure(text=now.strftime("%Y-%m-%d\n%H:%M:%S"))
                if self.clock_win and tk.Toplevel.winfo_exists(self.clock_win):
